...
After you finish physically building your car, take it to a lab assistant. They will help you select a unique network name for your vehicle (like "BYU-Car-1" or "Fast-Car") and program it into the `CameraWebServer.ino` file using the Arduino IDE.
...
Disconnect the car from the computer and power it using the onboard battery pack (4 AA batteries). Flip the power switch to the ON position. Open the Wi-Fi settings on your smartphone or computer, look for your custom network name, and connect to it. It is an open network, so no password is required.
...
1. **Remove the Wheels:** Detach the wheels from the motor shafts to prevent damage during transport.
2. **Pack Carefully:** Once the wheels are removed, your car should fit easily in a suitcase or storage container.
